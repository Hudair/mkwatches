		...
		foreach ( $attributes as $attribute ) {
			...
			if ( sanitize_title( $attribute['name'] ) == $product_attributes ) {
				...
				if ( $attribute['is_taxonomy'] ) {
					...
					} elseif ( function_exists( 'wcboost_variation_swatches' ) ) {
						$attr = $this->get_tax_attribute( $attribute['name'] );
						if ( $attr ) {
							$attr_type = $attr->attribute_type;
						}
					}
...
if( ! function_exists( 'woodstock_set_customer_session' ) ) {
	...
	if ( ! function_exists( 'woodstock_single_product_variations_clear_button' ) ) {
		/**
		 * Single product clear button
		 *
		 * @since 1.0.0
		 */
		function woodstock_single_product_variations_clear_button() {
			?>
		...
		add_action( 'woocommerce_before_single_variation', 'woodstock_single_product_variations_clear_button', 10 );
